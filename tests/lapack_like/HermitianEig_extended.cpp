  ...
  bool scalapack )
{
    ...
    if( scalapack && U == MC && V == MR )
    {
        if( onlyEigvals )
        {
            ...
            HermitianEig( uplo, ABlock, wBlock, subset, true );
            const double runTime = mpi::Time() - startTime;
            if( g.Rank() == 0 )
                Output("  ScaLAPACK HermitianEig (extended) time: ",runTime);
        }
        else
        {
            ...
            Matrix<Base<F>> wBlock;
            const double startTime = mpi::Time();
            HermitianEig( uplo, ABlock, wBlock, ZBlock, subset, true );
            const double runTime = mpi::Time() - startTime;
            if( g.Rank() == 0 )
                Output("  ScaLAPACK HermitianEig (extended) time: ",runTime);
        }
    ...
    const double startTime = mpi::Time();
    if( onlyEigvals )
        HermitianEig( uplo, A, w, sort, subset, ctrl, true );
    else
        HermitianEig( uplo, A, w, Z, sort, subset, ctrl, true );
    mpi::Barrier( g.Comm() );
    const double runTime = mpi::Time() - startTime;
    if( g.Rank() == 0 )
...
main( int argc, char* argv[] )
{
    ...
    try
    {
        ...
        ComplainIfDebug();
        if( commRank == 0 )
            Output("Will test HermitianEig (extended) ",uploChar);
