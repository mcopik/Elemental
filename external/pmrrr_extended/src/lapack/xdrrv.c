...
/* Subroutine */ int ext_xdrrv_(int *n, long double *vl, long double *vu, 
	long double *d__, long double *l, long double *pivmin, int *isplit, 
	int *m, int *dol, int *dou, long double *minrgp, 
	long double *rtol1, long double *rtol2, long double *w, long double *werr, 
	 long double *wgap, int *iblock, int *indexw, long double *gers, 
	 long double *z__, int *ldz, int *isuppz, long double *work, 
	int *iwork, int *info)
{
    ...
    long double gl;
    int im, in;
    long double gu, gap, eps, tau, tol, tmp;
    int zto;
    long double ztz;
    int iend, jblk;
    long double lgap;
    int done;
    long double rgap, left;
    int wend, iter;
    long double bstw;
    int itmp1;
    ...
    long double rqtol;
    int iindc1, iindc2;
    extern /* Subroutine */ int ext_xdr1v_(int *, int *, int *, 
    ...
    int needbs;
    int indlld;
    long double sgndef, mingma;
    extern /* Subroutine */ int ext_xdrrb_(int *, long double *, long double *, 
    ...
    long double spdiam;
    int negcnt;
    extern /* Subroutine */ int ext_xdrrf_(int *, long double *, long double *, 
    ...
    int ndepth;
    long double ssigma;
    extern /* Subroutine */ int ext_xdset_(char *, int *, int *, 
...
/*  XDRRV computes the eigenvectors of the tridiagonal matrix */
...
/*  L       (input/output) LONG DOUBLE PRECISION array, dimension (N) */
...
/*  PIVMIN  (in) LONG DOUBLE PRECISION */
...
/*  MINRGP  (input) LONG DOUBLE PRECISION */
...
/*  W       (input/output) LONG DOUBLE PRECISION array, dimension (N) */
...
/*  WERR    (input/output) LONG DOUBLE PRECISION array, dimension (N) */
/*          The first M elements contain the semiwidth of the uncertainty */
/*          interval of the corresponding eigenvalue in W */

/*  WGAP    (input/output) LONG DOUBLE PRECISION array, dimension (N) */
...
/*  WORK    (workspace) LONG DOUBLE PRECISION array, dimension (12*N) */
...
/*          > 0:  A problem occured in XDRRV. */
/*          < 0:  One of the called subroutines signaled an internal problem. */
/*                Needs inspection of the corresponding parameter IINFO */
/*                for further information. */

/*          =-1:  Problem in XDRRB when refining a child's eigenvalues. */
/*          =-2:  Problem in XDRRF when computing the RRR of a child. */
...
/*          =-3:  Problem in XDRRB when refining a single eigenvalue */
...
/*     The width of the part of Z that is used */
    ...
    ext_xdset_("Full", n, &zusedw, &c_b5, &c_b5, &z__[zusedl * z_dim1 + 1], ldz);
...
/*        corresponding eigenvectors. */
	ext_xdcpy_(&im, &w[wbegin], &c__1, &work[wbegin], &c__1);
...
/*                       Get representation from the right end of Z array */
		    ...
		    ext_xdcpy_(&in, &z__[ibegin + j * z_dim1], &c__1, &d__[ibegin]
, &c__1);
		    i__3 = in - 1;
		    ext_xdcpy_(&i__3, &z__[ibegin + (j + 1) * z_dim1], &c__1, &l[
			    ibegin], &c__1);
		    sigma = z__[iend + (j + 1) * z_dim1];
/*                 Set the corresponding entries in Z to zero */
		    ext_xdset_("Full", &in, &c__2, &c_b5, &c_b5, &z__[ibegin + j 
			    * z_dim1], ldz);
		}
...
/*                 eigenvalues to the precision needed. */
		    ext_xdrrb_(&in, &d__[ibegin], &work[indlld + ibegin - 1], &p, 
...
/*                 WBEGIN-1+OLDLST are correctly computed in XDRRB. */
...
/* Computing MAX */
			d__1 = wgap[wbegin + oldfst - 2], d__2 = w[wbegin + 
				...
				2];
			wgap[wbegin + oldfst - 2] = fmaxl(d__1,d__2);
		    }
...
/* Computing MAX */
			...
			wgap[wbegin + oldlst - 1] = fmaxl(d__1,d__2);
		    }
...
/*                    as possible */
			...
			for (k = 1; k <= 2; ++k) {
			    ...
			    offset = indexw[wbegin] - 1;
			    ext_xdrrb_(&in, &d__[ibegin], &work[indlld + ibegin 
...
/*                    XDRRF needs LWORK = 2*N */
			ext_xdrrf_(&in, &d__[ibegin], &l[ibegin], &work[indld + 
				...
				z_dim1], &z__[ibegin + (newftt + 1) * z_dim1], 
				 &work[indwrk], &iinfo);
			if (iinfo == 0) {
/*                       a new RRR for the cluster was found by XDRRF */
...
/* Computing MAX */
			    d__1 = fabsl(left), d__2 = fabsl(right);
			    lgap = eps * fmaxl(d__1,d__2);
			} else {
			    lgap = wgap[windmn];
			}
			if (k == im) {
...
/*                    Given LAMBDA, compute the eigenvector. */
			L__1 = ! usedbs;
			ext_xdr1v_(&in, &c__1, &in, &lambda, &d__[ibegin], &l[
...
/*                    (omitted when Bisection has been used) */
			...
			if (resid > tol * gap && fabsl(rqcorr) > rqtol * fabsl(
				lambda) && ! usedbs) {
...
/*                          improve error angle by second step */
				L__1 = ! usedbs;
				ext_xdr1v_(&in, &c__1, &in, &lambda, &d__[ibegin]
...
/* L123: */
			...
			i__4 = zto - zfrom + 1;
			ext_xdscl_(&i__4, &nrminv, &z__[zfrom + windex * z_dim1], 
				&c__1);
L125:
/*                    Update W */
...
/* Computing MAX */
				...
				wgap[windmn] = fmaxl(d__1,d__2);
			    }
			    if (windex < wend) {
/* Computing MAX */
				d__1 = savgap, d__2 = w[windpl] - werr[windpl]
					 - w[windex] - werr[windex];
				wgap[windex] = fmaxl(d__1,d__2);
			    }
			}
			++idone;
		    }
