...
/* Subroutine */ 
int ext_xdebz_(int *ijob, int *nitmax, int *n, 
	int *mmax, int *minp, int *nbmin, long double *abstol, 
	long double *reltol, long double *pivmin, long double *d__, long double *
	e, long double *e2, int *nval, long double *ab, long double *c__, 
	int *mout, int *nab, long double *work, int *iwork, 
	int *info)
{
...
/*  XDEBZ contains the iteration loops which compute and use the */
...
/*          Other values will cause XDEBZ to return with INFO=-1. */
...
/*          are generated, then XDEBZ will quit with INFO=MMAX+1. */
...
/*             NAB(i,j)=N(AB(i,j)), from a previous call to XDEBZ with */
/*             IJOB=1. */
...
/*             distinctive value(s) before XDEBZ is called. */
...
/*  (a) finding eigenvalues.  In this case, XDEBZ should have one or */
/*      more initial intervals set up in AB, and XDEBZ should be called */
/*      with IJOB=1.  This sets up NAB, and also counts the eigenvalues. */
/*      Intervals with no eigenvalues would usually be thrown out at */
/*      this point.  Also, if not all the eigenvalues in an interval i */
/*      are desired, NAB(i,1) can be increased or NAB(i,2) decreased. */
/*      For example, set NAB(i,1)=NAB(i,2)-1 to get the largest */
/*      eigenvalue.  XDEBZ is then called with IJOB=2 and MMAX */
...
/*      interval does not lie in (a,b).  XDEBZ is then called with */
...
/*        Check for convergence */
	...
	i__2 = kl;
	for (ji = kf; ji <= i__2; ++ji) {
	    tmp1 = (d__1 = ab[ji + (ab_dim1 << 1)] - ab[ji + ab_dim1], fabsl(
		    d__1));
/* Computing MAX */
	    d__3 = (d__1 = ab[ji + (ab_dim1 << 1)], fabsl(d__1)), d__4 = (d__2 =
		     ab[ji + ab_dim1], fabsl(d__2));
	    tmp2 = fmaxl(d__3,d__4);
/* Computing MAX */
	    d__1 = fmaxl(*abstol,*pivmin), d__2 = *reltol * tmp2;
	    if (tmp1 < fmaxl(d__1,d__2) || nab[ji + nab_dim1] >= nab[ji + (
		    nab_dim1 << 1)]) {
