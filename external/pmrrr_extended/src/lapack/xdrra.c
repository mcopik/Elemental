...
/*  XDRRA sets any "small" off-diagonal elements to zero. */
...
/*  TNRM (input) LONG DOUBLE PRECISION */
...
/*        Criterion that guarantees relative accuracy */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
	    eabs = (d__1 = e[i__], fabsl(d__1));
	    if (eabs <= *spltol * sqrt((d__1 = d__[i__], fabsl(d__1))) * sqrt((
		    d__2 = d__[i__ + 1], fabsl(d__2)))) {
		e[i__] = 0.;
		e2[i__] = 0.;
		isplit[*nsplit] = i__;
		++(*nsplit);
