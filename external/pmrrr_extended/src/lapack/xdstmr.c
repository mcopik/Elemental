...
/*  XDSTMR computes selected eigenvalues and, optionally, eigenvectors */
...
/*  Notes: */
/*  1.XDSTMR works only on machines which follow IEEE-754 */
...
/*          no error message related to NZC is issued by XRBLA. */
...
/*          message related to LWORK is issued by XRBLA. */
...
/*          no error message related to LIWORK is issued by XRBLA. */

/*  INFO    (output) INT */
/*          On exit, INFO */
/*          = 0:  successful exit */
/*          < 0:  if INFO = -i, the i-th argument had an illegal value */
/*          > 0:  if INFO = 1X, internal error in XDRRE, */
/*                if INFO = 2X, internal error in XDRRV. */
/*                Here, the digit X = ABS( IINFO ) < 10, where IINFO is */
/*                the nonzero error code returned by XDRRE or */
/*                XDRRV, respectively. */
...
/*     Test the input parameters. */
    ...
    zquery = *nzc == -1;
/*     XDSTMR needs WORK of size 6*N, IWORK of size 3*N. */
/*     In addition, XDRRE needs WORK of size 6*N, IWORK of size 5*N. */
/*     Furthermore, XDRRV needs WORK of size 12*N, IWORK of size 7*N. */
    if (wantz) {
	lwmin = *n * 18;
	liwmin = *n * 10;
    } else {
...
/*        It is either given by the user or computed in XDRRE. */
	wl = *vl;
	wu = *vu;
    } else if (indeig) {
...
/*           WANTZ .EQ. FALSE. */
	...
	if (zquery && *info == 0) {
	    z__[z_dim1 + 1] = (long double) nzcmin;
...
/*     are small */
/*     THRESH is the splitting parameter for XDRRE */
...
/*     Set the tolerance parameters for bisection */
    if (! wantz) {
/*        XDRRE computes the eigenvalues to full precision. */
	rtol1 = eps * 4.;
	rtol2 = eps * 4.;
    } else {
/*        XDRRE computes the eigenvalues to less than full precision. */
/*        XDRRV will refine the eigenvalue approximations, and we can */
/*        need less accurate initial bisection in XDRRE. */
/*        Note: these settings do only affect the subset case and XDRRE */
	rtol1 = sqrt(eps);
/* Computing MAX */
...
/*     Note that if RANGE .NE. 'V', XDRRE computes bounds on the desired */
...
/*        XDRRE computes eigenvalues of the (shifted) root representation */
/*        XDRRV returns the eigenvalues of the unshifted matrix. */
/*        However, if the eigenvectors are not desired by the user, we need */
/*        to apply the corresponding shifts from XDRRE to obtain the */
